...
def last_float(s: pd.Series) -> float:
    """Devuelve el último valor como float, o np.nan."""
...
# =========================
# Cálculo por timeframe (versión segura)
# =========================
def compute_tf(ticker: str, tf_key: str) -> str:
    ...
    # Periodo amplio para w/m/y; 1y para h/d
    ...
    bb_ma, bb_up, bb_lo, bb_pos = bollinger_position(close, 20, 2.0)
    ema20s = ema(close, 20)
    ...
    lines = [
        ...
        f"• RSI14: {rsi_v:.2f}" if not np.isnan(rsi_v) else "• RSI14: —",
        f"• MACD: {macd_label}",
    ]
